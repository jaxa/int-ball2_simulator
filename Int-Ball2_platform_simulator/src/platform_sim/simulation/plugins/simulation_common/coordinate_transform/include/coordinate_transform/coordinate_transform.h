...
namespace gazebo
{
	...
	public:
		 ...
		 */
		gz::math::Vector3d getLvlhPosFromWrold(const gz::math::Vector3d& world_pos);
		 ...
		 */
		gz::math::Vector3d getIssbodyPosFromLvlh(const gz::math::Vector3d& lvlh_pos, const gz::math::Quaterniond& iss_qtn);
		 ...
		 */
		gz::math::Vector3d getIssbodyVelFromLvlh(
		 ...
		 * @return                   JPM基準座標系での位置[m]
		 */
		gz::math::Vector3d getJpmRefPosFromWorld(const gz::math::Vector3d& world_pos, const gz::math::Quaterniond& iss_qtn);
		 ...
		 * @return                   JPM基準座標系での速度[m/s]
		 */
		gz::math::Vector3d getJpmRefVelFromWorld(
		 ...
		 * @return                   JPM基準座標系に対する姿勢クォータニオン
		 */
		gz::math::Quaterniond getJpmRefQtnFromWorld(const gz::math::Quaterniond& ib2_qtn, const gz::math::Quaterniond& iss_qtn);
		 ...
		 * @return                  ドッキングステーション座標系での位置[m]
		 */
		gz::math::Vector3d getDsPosFromWorld(const gz::math::Vector3d& world_pos, const gz::math::Quaterniond& iss_qtn);
		 ...
		 * @return                   ドッキングステーション座標系での速度[m/s]
		 */
		gz::math::Vector3d getDsVelFromWorld(
			const gz::math::Vector3d&    world_pos, const gz::math::Vector3d& world_vel,
			const gz::math::Quaterniond& iss_qtn,   const gz::math::Vector3d& iss_w
		);
		 ...
		 * @return                   ドッキングステーション座標系に対する姿勢クォータニオン
		 */
		gz::math::Quaterniond getDsQtnFromWorld(const gz::math::Quaterniond& ib2_qtn, const gz::math::Quaterniond& iss_qtn);

		/** LVLH(ISS座標系)での位置をWorld座標系へ変換
		 * @param [in] lvlh_pos      LVLH(ISS座標系)での位置[m]
		 * @return                   World座標系での位置[m]
		 */
		gz::math::Vector3d getWorldPosFromLvlh(const gz::math::Vector3d& lvlh_pos);
		 ...
		 * @return                   LVLH(ISS座標系)での位置[m]
		 */
		gz::math::Vector3d getLvlhPosFromIssbody(const gz::math::Vector3d& iss_body_pos, const gz::math::Quaterniond& iss_qtn);

		/** JPM基準座標系での位置をISS機体座標系へ変換
		 * @param [in] jpm_ref_pos   JPM基準座標系での位置[m]
		 * @return                   ISS機体座標系での位置[m]
		 */
		gz::math::Vector3d getIssbodyPosFromJpmRef(const gz::math::Vector3d& jpm_ref_pos);
		 ...
		 * @return                   World座標系での位置[m]
		 */
		gz::math::Vector3d getWorldPosFromJpmRef(const gz::math::Vector3d& jpm_ref_pos, const gz::math::Quaterniond& iss_qtn);

		//----------------------------------------------------------------------
		// メンバー変数
	private:
		/** World座標系でのISS重心位置 */
		gz::math::Vector3d          iss_cg_;

		/** ISS機体座標系でのJPM基準点の位置 */
		gz::math::Vector3d          jpm_pos_;

		/** ISS機体座標系でのJPMの姿勢(Euler) */
		gz::math::Vector3d          jpm_euler_;

		/** ISS機体座標系でのJPMの姿勢(Quaternion) */
		gz::math::Quaterniond       jpm_qtn_;

		/** JPM基準座標系でのドッキングステーション(ホーム座標系)原点 */
		gz::math::Vector3d          ds_pos_;

		/** JPM基準座標系でのドッキングステーション姿勢(Euler) */
		gz::math::Vector3d          ds_euler_;

		/** JPM基準座標系でのドッキングステーション姿勢(Quaternion) */
		gz::math::Quaterniond       ds_qtn_;
	};
}
