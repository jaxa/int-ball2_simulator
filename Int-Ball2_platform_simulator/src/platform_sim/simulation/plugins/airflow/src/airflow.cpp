
#include "airflow/airflow.h"

#include <gz/sim/Model.hh>
#include <gz/sim/Link.hh>
#include <gz/sim/Util.hh>
#include <gz/sim/components/Name.hh>
#include <gz/sim/components/Model.hh>
#include <gz/plugin/Register.hh>

#include <cfloat>
#include <iomanip>

namespace
{
	const double EPS(1.0E-10);
	const double TOL(0.1);
	const int K(216);
	const int VNUM(8);
	const int LNUM(6);
}

//------------------------------------------------------------------------------
// デフォルトコンストラクタ
airflow_plugin::Airflow::Airflow()
{
	wind_v_lp_.resize(LNUM);
}

//------------------------------------------------------------------------------
// デストラクタ.
airflow_plugin::Airflow::~Airflow() = default;

//------------------------------------------------------------------------------
// プラグインの初期設定
void airflow_plugin::Airflow::Configure(
	const gz::sim::Entity &/*_entity*/,
	const std::shared_ptr<const sdf::Element> &/*_sdf*/,
	gz::sim::EntityComponentManager &/*_ecm*/,
	gz::sim::EventManager &/*_eventMgr*/)
{
	if (!rclcpp::ok())
	{
		rclcpp::init(0, nullptr);
	}

	// Create ROS node
	ros_node_ = std::make_shared<rclcpp::Node>("airflow");

	// Publishers
	pub_drag_ = ros_node_->create_publisher<geometry_msgs::msg::WrenchStamped>("/airflow/drag", 1);
	pub_wind_ = ros_node_->create_publisher<geometry_msgs::msg::TwistStamped>("/airflow/wind", 1);

	// Airflow Parameter Update Server
	af_param_server_ = ros_node_->create_service<sim_msgs::srv::UpdateParameter>(
		"/sim/airflow/update_params",
		std::bind(&Airflow::updateParameter, this,
			std::placeholders::_1, std::placeholders::_2));

	// Get Parameters
	getParameter();

	// Read Fd Table
	readFdTable();
}

//------------------------------------------------------------------------------
// ROS Parameter Serverからパラメータ取得
void airflow_plugin::Airflow::getParameter()
{
	double x = 0.0, y = 0.0, z = 0.0;

	auto get_param = [this](const std::string &name, auto &value) {
		using T = std::decay_t<decltype(value)>;
		if (!ros_node_->has_parameter(name)) {
			ros_node_->declare_parameter<T>(name, value);
		}
		ros_node_->get_parameter(name, value);
	};

	get_param("model_name.iss_name", iss_name_);
	get_param("model_name.ib2_name", ib2_name_);

	x = 0.0; y = 0.0; z = 0.0;
	get_param("jpm_pose.pos.x", x);
	get_param("jpm_pose.pos.y", y);
	get_param("jpm_pose.pos.z", z);
	jpm_pos_.Set(x, y, z);

	x = 0.0; y = 0.0; z = 0.0;
	get_param("jpm_pose.att.r", x);
	get_param("jpm_pose.att.p", y);
	get_param("jpm_pose.att.y", z);
	jpm_att_.Set(x, y, z);
	jpm_att_ = jpm_att_ * (M_PI / 180.0);

	load_point_.clear();
	for (int i = 0; i < LNUM; i++)
	{
		gz::math::Vector3d p;
		std::string str = "airflow_parameter.load_point.p" + std::to_string(i);
		x = 0.0; y = 0.0; z = 0.0;
		get_param(str + ".x", x);
		get_param(str + ".y", y);
		get_param(str + ".z", z);
		p.Set(x, y, z);
		load_point_.push_back(p);
	}

	x = 0.0; y = 0.0; z = 0.0;
	get_param("robot_mass_property.cg.x", x);
	get_param("robot_mass_property.cg.y", y);
	get_param("robot_mass_property.cg.z", z);
	cg_.Set(x, y, z);

	get_param("robot_mass_property.radius", radius_);
	S_ = 4.0 * M_PI * radius_ * radius_;

	get_param("airflow_parameter.force_stddev", force_stddev_);
	get_param("airflow_parameter.torque_stddev", torque_stddev_);
	get_param("airflow_parameter.kappa", kappa_);
	get_param("airflow_parameter.pcd", pcd_);
	get_param("airflow_parameter.time_step", time_step_);
	get_param("airflow_parameter.table", table_);
	get_param("airflow_parameter.get_cycle", get_cycle_);
	get_param("airflow_parameter.pub_cycle", pub_cycle_);

	plugin_path_ = "";
	get_param("airflow_parameter.plugin_path", plugin_path_);

	// 乱数のシード値
	int seed = -1;
	get_param("sim_common.random_seed", seed);
	if (seed >= 0)
	{
		RCLCPP_INFO(ros_node_->get_logger(), "Set the random seed value %d", seed);
		gz::math::Rand::Seed(static_cast<unsigned int>(seed));
	}

	logParameter();
}

//------------------------------------------------------------------------------
// パラメータ更新サービス
void airflow_plugin::Airflow::updateParameter(
	const std::shared_ptr<sim_msgs::srv::UpdateParameter::Request> /*req*/,
	std::shared_ptr<sim_msgs::srv::UpdateParameter::Response> res)
{
	auto get_param = [this](const std::string &name, auto &value) {
		using T = std::decay_t<decltype(value)>;
		if (!ros_node_->has_parameter(name)) {
			ros_node_->declare_parameter<T>(name, value);
		}
		ros_node_->get_parameter(name, value);
	};

	get_param("airflow_parameter.force_stddev", force_stddev_);
	get_param("airflow_parameter.torque_stddev", torque_stddev_);

	res->result = true;
}

//------------------------------------------------------------------------------
// Model取得
void airflow_plugin::Airflow::getModels(gz::sim::EntityComponentManager &_ecm)
{
	if (iss_model_ == gz::sim::kNullEntity && !iss_name_.empty())
	{
		iss_model_ = _ecm.EntityByComponents(
			gz::sim::components::Name(iss_name_),
			gz::sim::components::Model());
		if (iss_model_ != gz::sim::kNullEntity)
		{
			gz::sim::Model model(iss_model_);
			auto links = model.Links(_ecm);
			if (!links.empty())
				iss_link_ = links[0];
		}
	}

	if (ib2_model_ == gz::sim::kNullEntity && !ib2_name_.empty())
	{
		ib2_model_ = _ecm.EntityByComponents(
			gz::sim::components::Name(ib2_name_),
			gz::sim::components::Model());
		if (ib2_model_ != gz::sim::kNullEntity)
		{
			gz::sim::Model model(ib2_model_);
			auto links = model.Links(_ecm);
			if (!links.empty())
				ib2_link_ = links[0];
		}
	}
}

//------------------------------------------------------------------------------
// 物理ステップ前の更新
void airflow_plugin::Airflow::PreUpdate(
	const gz::sim::UpdateInfo &_info,
	gz::sim::EntityComponentManager &_ecm)
{
	if (_info.paused)
		return;

	rclcpp::spin_some(ros_node_);

	getModels(_ecm);

	if (ib2_link_ == gz::sim::kNullEntity || iss_link_ == gz::sim::kNullEntity)
		return;

	// Enable velocity checks on first call
	if (!velocity_checks_enabled_)
	{
		gz::sim::Link ib2_lnk(ib2_link_);
		ib2_lnk.EnableVelocityChecks(_ecm);
		gz::sim::Link iss_lnk(iss_link_);
		iss_lnk.EnableVelocityChecks(_ecm);
		velocity_checks_enabled_ = true;
		return;
	}

	airflowCallBack(_info, _ecm);
}

//------------------------------------------------------------------------------
// airflowコールバック
void airflow_plugin::Airflow::airflowCallBack(
	const gz::sim::UpdateInfo &_info,
	gz::sim::EntityComponentManager &_ecm)
{
	gz::math::Vector3d force_cg_j;
	gz::math::Vector3d force_cg_b;
	gz::math::Vector3d torque_b;

	sim_time_ = std::chrono::duration<double>(_info.simTime).count();

	loadWindVecField();
	getRobotState(_ecm);
	getPointIndex();

	if (cloud_now_.empty())
		return;

	// ロボットの重心に作用する風速ベクトル
	getWindVec(state_.pos, wind_v_cg_);
	// ロボットの作用点に作用する風速ベクトル
	for (int i = 0; i < LNUM; i++)
	{
		auto point = state_.pos - state_.rot.RotateVector(cg_) + load_point_[i];
		getWindVec(point, wind_v_lp_[i]);
	}

	getForce(wind_v_cg_, force_cg_j);
	getTorque(wind_v_lp_, torque_b);

	force_cg_b = state_.rot.RotateVectorReverse(force_cg_j);
	addForceAndTorque(_ecm, force_cg_b, torque_b);
	pubWrenchAndWind(_info, force_cg_j, torque_b, wind_v_cg_);
}

//------------------------------------------------------------------------------
// JPM基準座標系でのロボット状態量を取得
void airflow_plugin::Airflow::getRobotState(gz::sim::EntityComponentManager &_ecm)
{
	gz::sim::Link ib2_lnk(ib2_link_);
	gz::sim::Link iss_lnk(iss_link_);

	auto ib2_pose = ib2_lnk.WorldPose(_ecm);
	auto ib2_vel  = ib2_lnk.WorldLinearVelocity(_ecm);
	auto iss_pose = iss_lnk.WorldPose(_ecm);

	if (!ib2_pose || !iss_pose)
		return;

	auto iss_cg = gz::math::Vector3d(iss_pose->Pos().X(), iss_pose->Pos().Y(), iss_pose->Pos().Z());
	coord_transformer_.set(iss_cg, jpm_pos_, jpm_att_);

	// Position
	auto iss_qtn   = gz::math::Quaterniond(iss_pose->Rot().W(), iss_pose->Rot().X(),
	                                        iss_pose->Rot().Y(), iss_pose->Rot().Z());
	auto world_pos = gz::math::Vector3d(ib2_pose->Pos().X(), ib2_pose->Pos().Y(), ib2_pose->Pos().Z());
	state_.pos     = coord_transformer_.getJpmRefPosFromWorld(world_pos, iss_qtn);

	// Velocity
	auto iss_worldAngVel = iss_lnk.WorldAngularVelocity(_ecm);
	gz::math::Vector3d iss_w = gz::math::Vector3d::Zero;
	if (iss_worldAngVel)
	{
		iss_w = iss_pose->Rot().RotateVectorReverse(*iss_worldAngVel);
	}
	gz::math::Vector3d world_vel = ib2_vel ? *ib2_vel : gz::math::Vector3d::Zero;
	state_.vel = coord_transformer_.getDsVelFromWorld(world_pos, world_vel, iss_qtn, iss_w);

	// Quaternion
	auto world_qtn = gz::math::Quaterniond(ib2_pose->Rot().W(), ib2_pose->Rot().X(),
	                                        ib2_pose->Rot().Y(), ib2_pose->Rot().Z());
	state_.rot = coord_transformer_.getDsQtnFromWorld(world_qtn, iss_qtn);
}

//------------------------------------------------------------------------------
// PCDファイルからのJEM船内風速ベクトル場取得
void airflow_plugin::Airflow::loadWindVecField()
{
	std::string pcd_file = plugin_path_ + pcd_;

	int step       = int(sim_time_ * 1000) / time_step_;
	int msec_now   = step       * time_step_;
	int msec_next1 = msec_now   + time_step_;
	int msec_next2 = msec_next1 + time_step_;

	if (current_postfix_ == msec_now)
	{
		return;
	}

	// Read PCD File
	if (msec_now == 0)
	{
		std::ostringstream ossnow;
		ossnow << std::setw(8) << std::setfill('0') << msec_now;
		std::string file_name_now = pcd_file + ossnow.str() + ".pcd";
		pcl::io::loadPCDFile(file_name_now, cloud_now_);

		std::ostringstream ossnext1;
		ossnext1 << std::setw(8) << std::setfill('0') << msec_next1;
		std::string file_name_next1 = pcd_file + ossnext1.str() + ".pcd";
		pcl::io::loadPCDFile(file_name_next1, cloud_next1_);
	}
	else
	{
		cloud_now_   = cloud_next1_;
		cloud_next1_ = cloud_next2_;
	}

	current_postfix_ = msec_now;

	std::ostringstream ossnext2;
	ossnext2 << std::setw(8) << std::setfill('0') << msec_next2;
	std::string file_name_next2 = pcd_file + ossnext2.str() + ".pcd";
	pcl::io::loadPCDFile(file_name_next2, cloud_next2_);
}

//------------------------------------------------------------------------------
// K近傍点のポイントインデックスを取得
void airflow_plugin::Airflow::getPointIndex()
{
	if (cloud_now_.empty())
		return;

	if (get_cnt_ == 0)
	{
		if (nnSearch(state_.pos, point_idx_) < VNUM)
		{
			RCLCPP_ERROR(ros_node_->get_logger(), "Not Found at least 8-Nearest Neighbor");
		}
	}
	else if (get_cnt_ >= static_cast<int>(get_cycle_ * 1000 + TOL) - 1)
	{
		get_cnt_ = -1;
	}
	get_cnt_++;
}

//------------------------------------------------------------------------------
// K-Nearest Neighbors Search
int airflow_plugin::Airflow::nnSearch(
	const gz::math::Vector3d& point, std::vector<int>& point_idx)
{
	point_idx.clear();

	float resolution = 0.1f;
	std::vector<float> point_distance;
	pcl::PointWithViewpoint search_point;
	pcl::octree::OctreePointCloudSearch<pcl::PointWithViewpoint> octree(resolution);

	octree.setInputCloud(cloud_now_.makeShared());
	octree.addPointsFromInputCloud();
	search_point.x = static_cast<float>(point.X());
	search_point.y = static_cast<float>(point.Y());
	search_point.z = static_cast<float>(point.Z());

	return octree.nearestKSearch(search_point, K, point_idx, point_distance);
}

//------------------------------------------------------------------------------
// Select (8) Vertices that contain a Robot CG Position
bool airflow_plugin::Airflow::selectVertex(
	const gz::math::Vector3d& point,
	const std::vector<pcl::PointWithViewpoint>& in_vertices,
	      std::vector<pcl::PointWithViewpoint>& out_vertices)
{
	int size = static_cast<int>(in_vertices.size());
	std::vector<Vertex> vertex(size);

	for (int i = 0; i < size; i++)
	{
		double dx = point.X() - in_vertices[i].x;
		double dy = point.Y() - in_vertices[i].y;
		double dz = point.Z() - in_vertices[i].z;

		vertex[i].distance = sqrt(dx * dx + dy * dy + dz * dz);
		vertex[i].point    = in_vertices[i];

		vertex[i].is_positive_x = (dx < 0);
		vertex[i].is_positive_y = (dy < 0);
		vertex[i].is_positive_z = (dz < 0);
	}

	double dmin[VNUM];
	for (int i = 0; i < VNUM; i++)
	{
		dmin[i] = DBL_MAX;
	}

	for (int i = 0; i < size; i++)
	{
		unsigned char point_num = ((vertex[i].is_positive_z << 2) |
		                           (vertex[i].is_positive_y << 1) |
		                           (vertex[i].is_positive_x << 0));

		for (int pn = 0; pn < VNUM; pn++)
		{
			if (pn == static_cast<int>(point_num))
			{
				if (vertex[i].distance < dmin[pn])
				{
					dmin[pn]         = vertex[i].distance;
					out_vertices[pn] = vertex[i].point;
				}
			}
		}
	}

	for (int i = 0; i < VNUM; i++)
	{
		if (std::abs(dmin[i] - DBL_MAX) < EPS)
		{
			return false;
		}
	}

	return true;
}

//------------------------------------------------------------------------------
// 線形補間
gz::math::Vector3d airflow_plugin::Airflow::lerp(
	const std::vector<pcl::PointWithViewpoint>& vertex,
	const gz::math::Vector3d& point)
{
	int size = static_cast<int>(vertex.size());
	std::vector<gz::math::Vector3d> v(size);

	for (int i = 0; i < size; i++)
	{
		v[i].X() = vertex[i].vp_x;
		v[i].Y() = vertex[i].vp_y;
		v[i].Z() = vertex[i].vp_z;
	}

	// Interpolate on -Z side
	double x10 = vertex[1].x - vertex[0].x;
	double x32 = vertex[3].x - vertex[2].x;
	double x1w = vertex[1].x - point.X();
	double xw0 = point.X()   - vertex[0].x;
	double x3w = vertex[3].x - point.X();
	double xw2 = point.X()   - vertex[2].x;
	double y01 = (x1w * vertex[0].y + xw0 * vertex[1].y) / x10;
	double z01 = (x1w * vertex[0].z + xw0 * vertex[1].z) / x10;
	double y23 = (x3w * vertex[2].y + xw2 * vertex[3].y) / x32;
	double z23 = (x3w * vertex[2].z + xw2 * vertex[3].z) / x32;

	auto v01 = (x1w * v[0] + xw0 * v[1]) / x10;
	auto v23 = (x3w * v[2] + xw2 * v[3]) / x32;
	auto v03 = ((y23 - point.Y()) * v01 + (point.Y() - y01) * v23) / (y23 - y01);

	// Interpolate on +Z side
	double x54 = vertex[5].x - vertex[4].x;
	double x76 = vertex[7].x - vertex[6].x;
	double x5w = vertex[5].x - point.X();
	double xw4 = point.X()   - vertex[4].x;
	double x7w = vertex[7].x - point.X();
	double xw6 = point.X()   - vertex[6].x;
	double y45 = (x5w * vertex[4].y + xw4 * vertex[5].y) / x54;
	double z45 = (x5w * vertex[4].z + xw4 * vertex[5].z) / x54;
	double y67 = (x7w * vertex[6].y + xw6 * vertex[7].y) / x76;
	double z67 = (x7w * vertex[6].z + xw6 * vertex[7].z) / x76;

	auto v45 = (x5w * v[4] + xw4 * v[5]) / x54;
	auto v67 = (x7w * v[6] + xw6 * v[7]) / x76;
	auto v47 = ((y67 - point.Y()) * v45 + (point.Y() - y45) * v67) / (y67 - y45);

	// Calculate Wind Vector at Robot position
	double z03 = ((y23 - point.Y()) * z01 + (point.Y() - y01) * z23) / (y23 - y01);
	double z47 = ((y67 - point.Y()) * z45 + (point.Y() - y45) * z67) / (y67 - y45);

	auto vw = ((z47 - point.Z()) * v03 + (point.Z() - z03) * v47) / (z47 - z03);

	return vw;
}

//------------------------------------------------------------------------------
// 抗力テーブル読み込み
void airflow_plugin::Airflow::readFdTable()
{
	std::string file_name = plugin_path_ + table_;

	std::ifstream ifs;
	ifs.open(file_name, std::ios::in);
	if (!ifs)
	{
		RCLCPP_ERROR(ros_node_->get_logger(), "Cannot Open %s", file_name.c_str());
		return;
	}

	std::string line;
	while (getline(ifs, line))
	{
		if (line.find_first_of('#') == 0 || line.length() == 0)
		{
			continue;
		}

		double tbl[2];
		std::replace(line.begin(), line.end(), ',', ' ');
		std::istringstream iss(line);
		iss >> tbl[0] >> tbl[1];
		fd_table_.push_back({tbl[0], tbl[1]});
	}

	ifs.close();
}

//------------------------------------------------------------------------------
// JPM基準座標系での風速ベクトル取得
void airflow_plugin::Airflow::getWindVec(
	const gz::math::Vector3d& point, gz::math::Vector3d& wind_v)
{
	if (point_idx_.empty() || cloud_now_.empty())
	{
		wind_v = gz::math::Vector3d::Zero;
		return;
	}

	std::vector<pcl::PointWithViewpoint> pv_vec_now;
	std::vector<pcl::PointWithViewpoint> pv_vec_next;
	for (int i = 0; i < K && i < static_cast<int>(point_idx_.size()); i++)
	{
		pv_vec_now.push_back(cloud_now_.points[point_idx_[i]]);
		pv_vec_next.push_back(cloud_next1_.points[point_idx_[i]]);
	}

	std::vector<pcl::PointWithViewpoint> vertex_now(VNUM);
	std::vector<pcl::PointWithViewpoint> vertex_next(VNUM);
	if (!selectVertex(point, pv_vec_now, vertex_now) ||
	    !selectVertex(point, pv_vec_next, vertex_next))
	{
		RCLCPP_WARN(ros_node_->get_logger(),
			"Point (%f, %f, %f) is Out of Area prepared by PCD file",
			point.X(), point.Y(), point.Z());
		wind_v = gz::math::Vector3d::Zero;
		return;
	}

	// Linear Interpolation
	auto wind_now  = lerp(vertex_now,  point);
	auto wind_next = lerp(vertex_next, point);

	double l = sim_time_ - static_cast<double>(current_postfix_) / 1000.0;
	double m = -(sim_time_ - static_cast<double>(current_postfix_ + time_step_) / 1000.0);

	wind_v = (m * wind_now + l * wind_next) / (time_step_ / 1000.0);
}

//------------------------------------------------------------------------------
// 抗力を取得する
void airflow_plugin::Airflow::getForce(
	const gz::math::Vector3d& wind_v, gz::math::Vector3d& force)
{
	auto   rel_vel(state_.vel - wind_v);
	double rel_vel_norm = rel_vel.Length();

	if (std::abs(rel_vel_norm) < EPS)
	{
		force = gz::math::Vector3d::Zero;
		return;
	}

	double drag_force_norm = 0;
	for (int i = 0; i < static_cast<int>(fd_table_.size()) - 1; i++)
	{
		if (fd_table_[i][0] <= rel_vel_norm && rel_vel_norm < fd_table_[i + 1][0])
		{
			double l   = -(fd_table_[i][0]     - rel_vel_norm);
			double m   =   fd_table_[i + 1][0] - rel_vel_norm;
			double num =   fd_table_[i + 1][0] - fd_table_[i][0];

			if (num < EPS)
			{
				RCLCPP_ERROR(ros_node_->get_logger(), "Divided By 0 in Drag Plugin");
				return;
			}

			drag_force_norm = (l * fd_table_[i + 1][1] + m * fd_table_[i][1]) / num;
			continue;
		}

		if (i == static_cast<int>(fd_table_.size()) - 2 && drag_force_norm == 0.0)
		{
			drag_force_norm = fd_table_[i + 1][1];
			RCLCPP_WARN(ros_node_->get_logger(), "Relative Velocity is over %f", fd_table_[i + 1][0]);
		}
	}

	auto unit_wind_v(rel_vel);
	unit_wind_v.Normalize();

	double nx = gz::math::Rand::DblNormal(0.0, 1.0);
	double ny = gz::math::Rand::DblNormal(0.0, 1.0);
	double nz = gz::math::Rand::DblNormal(0.0, 1.0);
	force     = -drag_force_norm * unit_wind_v;
	force.X() = force.X() * (1.0 + force_stddev_ * nx);
	force.Y() = force.Y() * (1.0 + force_stddev_ * ny);
	force.Z() = force.Z() * (1.0 + force_stddev_ * nz);
}

//------------------------------------------------------------------------------
// 機体座標系でのトルク取得
void airflow_plugin::Airflow::getTorque(
	const std::vector<gz::math::Vector3d>& wind_v, gz::math::Vector3d& torque)
{
	int  size = static_cast<int>(wind_v.size());
	auto t    = gz::math::Vector3d::Zero;

	for (int i = 0; i < size; i++)
	{
		auto rel_vel_j = wind_v[i]      - state_.vel;
		auto arm_j     = load_point_[i] - state_.rot.RotateVector(cg_);
		t              = t + arm_j.Cross(rel_vel_j) / arm_j.Length();
	}

	double nx = gz::math::Rand::DblNormal(0.0, 1.0);
	double ny = gz::math::Rand::DblNormal(0.0, 1.0);
	double nz = gz::math::Rand::DblNormal(0.0, 1.0);
	torque    = state_.rot.RotateVectorReverse(0.125 * radius_ * kappa_ * S_ * t);
	torque.X() = torque.X() * (1.0 + torque_stddev_ * nx);
	torque.Y() = torque.Y() * (1.0 + torque_stddev_ * ny);
	torque.Z() = torque.Z() * (1.0 + torque_stddev_ * nz);
}

//------------------------------------------------------------------------------
// エアフローによる抗力、トルクを印加する
void airflow_plugin::Airflow::addForceAndTorque(
	gz::sim::EntityComponentManager &_ecm,
	const gz::math::Vector3d& force_body,
	const gz::math::Vector3d& torque_body)
{
	gz::sim::Link link(ib2_link_);

	auto worldPose = link.WorldPose(_ecm);
	if (!worldPose)
		return;

	auto worldForce  = worldPose->Rot().RotateVector(force_body);
	auto worldTorque = worldPose->Rot().RotateVector(torque_body);

	link.AddWorldWrench(_ecm, worldForce, worldTorque);
}

//------------------------------------------------------------------------------
// エアフローによる抗力、トルクおよび風速ベクトルをパブリッシュ
void airflow_plugin::Airflow::pubWrenchAndWind(
	const gz::sim::UpdateInfo &_info,
	const gz::math::Vector3d& f,
	const gz::math::Vector3d& t,
	const gz::math::Vector3d& w)
{
	if (pub_cnt_ == 0)
	{
		auto sim_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(_info.simTime).count();

		geometry_msgs::msg::WrenchStamped wrench_stamped;
		wrench_stamped.header.stamp.sec     = static_cast<int32_t>(sim_ns / 1000000000);
		wrench_stamped.header.stamp.nanosec = static_cast<uint32_t>(sim_ns % 1000000000);
		wrench_stamped.wrench.force.x  = f.X();
		wrench_stamped.wrench.force.y  = f.Y();
		wrench_stamped.wrench.force.z  = f.Z();
		wrench_stamped.wrench.torque.x = t.X();
		wrench_stamped.wrench.torque.y = t.Y();
		wrench_stamped.wrench.torque.z = t.Z();
		pub_drag_->publish(wrench_stamped);

		geometry_msgs::msg::TwistStamped wind_stamped;
		wind_stamped.header.stamp.sec     = static_cast<int32_t>(sim_ns / 1000000000);
		wind_stamped.header.stamp.nanosec = static_cast<uint32_t>(sim_ns % 1000000000);
		wind_stamped.twist.linear.x  = w.X();
		wind_stamped.twist.linear.y  = w.Y();
		wind_stamped.twist.linear.z  = w.Z();
		pub_wind_->publish(wind_stamped);
	}
	else if (pub_cnt_ >= static_cast<int>(pub_cycle_ * 1000 + TOL) - 1)
	{
		pub_cnt_ = -1;
	}
	pub_cnt_++;
}

//------------------------------------------------------------------------------
// エアフロープラグインパラメータログ作成
void airflow_plugin::Airflow::logParameter()
{
	RCLCPP_INFO(ros_node_->get_logger(), "***************** Airflow Parameter");

	for (int i = 0; i < LNUM; i++)
	{
		RCLCPP_INFO(ros_node_->get_logger(), "load_point p%d: %f %f %f",
			i, load_point_[i].X(), load_point_[i].Y(), load_point_[i].Z());
	}

	RCLCPP_INFO(ros_node_->get_logger(), "force_stddev : %f", force_stddev_);
	RCLCPP_INFO(ros_node_->get_logger(), "torque_stddev: %f", torque_stddev_);
	RCLCPP_INFO(ros_node_->get_logger(), "kappa        : %f", kappa_);
	RCLCPP_INFO(ros_node_->get_logger(), "pcd          : %s", pcd_.c_str());
	RCLCPP_INFO(ros_node_->get_logger(), "time_step    : %d", time_step_);
	RCLCPP_INFO(ros_node_->get_logger(), "table        : %s", table_.c_str());
	RCLCPP_INFO(ros_node_->get_logger(), "get_cycle    : %f", get_cycle_);
	RCLCPP_INFO(ros_node_->get_logger(), "pub_cycle    : %f", pub_cycle_);
}

//------------------------------------------------------------------------------
// gz-simのシステムプラグインとして登録
GZ_ADD_PLUGIN(
	airflow_plugin::Airflow,
	gz::sim::System,
	gz::sim::ISystemConfigure,
	gz::sim::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(airflow_plugin::Airflow, "airflow::Airflow")

// End Of File -----------------------------------------------------------------
