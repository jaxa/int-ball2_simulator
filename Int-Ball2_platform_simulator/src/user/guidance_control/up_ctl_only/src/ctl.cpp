	...
	dtc_(this), seq_status_(0), valid_navigation_(false)
{
	...
	command_as_ = rclcpp_action::create_server<CtlCommandAction>(
		this, COMMAND_ACTION,
		std::bind(&Ctl::handleGoal, this, _1, _2),
		std::bind(&Ctl::handleCancel, this, _1),
		std::bind(&Ctl::handleAccepted, this, _1));

	// Service server
	update_ss_ = this->create_service<ib2_msgs::srv::UpdateParameter>(
		UPDATE_SERVICE,
		std::bind(&Ctl::updateCallback, this, _1, _2));

	// Service client
	marker_sc_ = this->create_client<ib2_msgs::srv::MarkerCorrection>("/sensor_fusion/marker_correction");

	// Subscribers
	navinfo_sub_ = this->create_subscription<ib2_msgs::msg::Navigation>(
		TOPIC_NAV_POSE, 5,
		std::bind(&Ctl::navinfoCallback, this, _1));
	up_wrench_mode_sub_ = this->create_subscription<std_msgs::msg::Int8>(
		TOPIC_UP_WRENCH_MODE, 1,
		std::bind(&Ctl::upWrenchModeCallback, this, _1));
	up_torque_gain_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
		TOPIC_UP_TORQUE_GAIN, 1,
		std::bind(&Ctl::upTorqueGainCallback, this, _1));

	// Publishers
	wrench_pub_  = this->create_publisher<geometry_msgs::msg::WrenchStamped>(TOPIC_CTL_WRENCH, 5);
	profile_pub_ = this->create_publisher<ib2_msgs::msg::CtlProfile>(TOPIC_CTL_PROFILE, 5);
	status_pub_  = this->create_publisher<ib2_msgs::msg::CtlStatus>(TOPIC_CTL_STATUS, 5);

	timer_ = this->create_wall_timer(
		std::chrono::nanoseconds(interval_status_.nanoseconds()),
		std::bind(&Ctl::timerCallback, this));

	up_torque_gain_ = Eigen::Vector3d::Zero(3);
	up_wrench_mode_ = UP_WRENCH_MODE::NORMAL;
}
...
bool Ctl::isGoalActive() const
{
	return current_goal_handle_ && current_goal_handle_->is_active();
}

bool Ctl::isPreemptRequested() const
{
	return current_goal_handle_ && current_goal_handle_->is_canceling();
}

void Ctl::publishFeedback(const CtlCommandAction::Feedback& fb)
{
	if (current_goal_handle_ && current_goal_handle_->is_active())
		current_goal_handle_->publish_feedback(
			std::make_shared<CtlCommandAction::Feedback>(fb));
}

void Ctl::setSucceeded(const CtlCommandAction::Result& r)
{
	if (current_goal_handle_ && current_goal_handle_->is_active())
		current_goal_handle_->succeed(
			std::make_shared<CtlCommandAction::Result>(r));
}

void Ctl::setPreempted(const CtlCommandAction::Result& r)
{
	if (current_goal_handle_ && current_goal_handle_->is_active())
		current_goal_handle_->canceled(
			std::make_shared<CtlCommandAction::Result>(r));
}

//------------------------------------------------------------------------------
// Action server callbacks
rclcpp_action::GoalResponse Ctl::handleGoal(
	const rclcpp_action::GoalUUID&,
	std::shared_ptr<const CtlCommandAction::Goal>)
{
	return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse Ctl::handleCancel(
	const std::shared_ptr<GoalHandleCtlCommand>)
{
	return rclcpp_action::CancelResponse::ACCEPT;
}

void Ctl::handleAccepted(const std::shared_ptr<GoalHandleCtlCommand> goal_handle)
{
...
bool Ctl::setMember()
{
	...
	try
	{
		...
		ib2::CtlBody body(this);
		ib2::PosController ctl_pos(this);
		ib2::AttController ctl_att(this);
		ib2::PosProfiler prof_pos(this);
		...
		auto declare_and_get = [this](const std::string& name, auto& val) {
			using T = std::decay_t<decltype(val)>;
			if (!this->has_parameter(name))
				this->declare_parameter(name, val);
			this->get_parameter(name, val);
		};

		declare_and_get(ROSPARAM_INTERVAL_STATUS   , interval_status);
		declare_and_get(ROSPARAM_INTERVAL_FEEDBACK , interval_feedback);
		declare_and_get(ROSPARAM_DURATION_GOAL     , duration_goal);
		declare_and_get(ROSPARAM_TOLERANCE_POS     , tolerance_pos);
		declare_and_get(ROSPARAM_TOLERANCE_ATT     , tolerance_att);
		declare_and_get(ROSPARAM_TOLERANCE_POS_STOP, tolerance_pos_stop);
		declare_and_get(ROSPARAM_TOLERANCE_ATT_STOP, tolerance_att_stop);
		declare_and_get(ROSPARAM_WAIT_CANCEL       , wait_cancel);
		declare_and_get(ROSPARAM_WAIT_RELEASE      , wait_release);
		declare_and_get(ROSPARAM_WAIT_CALIBRATION  , wait_calibration);
		declare_and_get(ROSPARAM_WAIT_DOCKING      , wait_docking);
		declare_and_get(ROSPARAM_NAV_COUNTER       , nav_counter);
		declare_and_get(ROSPARAM_NAV_DR            , nav_dr);
		declare_and_get(ROSPARAM_NAV_DV            , nav_dv);
		declare_and_get(ROSPARAM_NAV_DA            , nav_da);
		declare_and_get(ROSPARAM_NAV_DQ            , nav_dq);
		declare_and_get(ROSPARAM_NAV_DW            , nav_dw);
		...
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_INTERVAL_STATUS.c_str()   , interval_status_.seconds());
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_INTERVAL_FEEDBACK.c_str() , interval_feedback_.seconds());
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_DURATION_GOAL.c_str()     , duration_goal_.seconds());
		...
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_WAIT_CANCEL.c_str()       , waitCancel_.seconds());
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_WAIT_RELEASE.c_str()      , waitRelease_.seconds());
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_WAIT_CALIBRATION.c_str()  , waitCalibration_.seconds());
		RCLCPP_INFO(this->get_logger(), "%s   : %f", ROSPARAM_WAIT_DOCKING.c_str()      , waitDocking_.seconds());
...
{
	rclcpp::Time tnav(last_nav_stamp_.pose.header.stamp);
	auto tfb = tnav;
	auto tbGoal = tnav;
	bool stayGoal(false);
	while (true)
	{
		...
		{
			cancelTarget();
			abortAction(CtlCommandAction::Result::TERMINATE_ABORTED);
			break;
		}
		if (isPreemptRequested() || !rclcpp::ok())
		{
			abortAction(CtlCommandAction::Result::TERMINATE_ABORTED);
			if (isGoalActive())
				status_ = ib2_msgs::msg::CtlStatusType::STAND_BY;
...
//------------------------------------------------------------------------------
// ターゲットモードの処理
void Ctl::target(const std::shared_ptr<const CtlCommandAction::Goal>& goal)
{
...
void Ctl::release()
{
	if (status_ != ib2_msgs::msg::CtlStatusType::STAND_BY)
		abortAction(CtlCommandAction::Result::TERMINATE_INVALID_CMD);
	else
	{
		setKeepPose();
		controller_->flash();
...
void Ctl::docking(bool correction)
{
		 ...
		 status_ <= ib2_msgs::msg::CtlStatusType::MOVING_TO_RDP; ++status_)
	{
		...
		else if (status_ == ib2_msgs::msg::CtlStatusType::MOVING_TO_AIA_AIP)
		{
			...
			auto future = marker_sc_->async_send_request(request);
			bool corrected = false;
			if (future.wait_for(std::chrono::seconds(5)) == std::future_status::ready)
			{
				auto response = future.get();
				corrected = (response->status ==
							 ib2_msgs::srv::MarkerCorrection::Response::SUCCESS);
			}
			if (!corrected)
			{
				abortAction(CtlCommandAction::Result::TERMINATE_INVALID_NAV);
				break;
			}
		}
...
void Ctl::dockingStandBy()
{
	...
	status_ = ib2_msgs::msg::CtlStatusType::STAND_BY;
	abortAction(aborted ? CtlCommandAction::Result::TERMINATE_ABORTED :
				CtlCommandAction::Result::TERMINATE_TIME_OUT);
}
...
void Ctl::scan()
{
	...
	for (size_t i = 0; i < nscan; ++i)
	{
		...
		{
			CtlCommandAction::Result r;
			r.stamp = this->now();
			r.type = CtlCommandAction::Result::TERMINATE_INVALID_NAV;
			setSucceeded(r);
			status_ = ib2_msgs::msg::CtlStatusType::STAND_BY;
		}
	}
...
void Ctl::goalTarget()
{
	...
	CtlCommandAction::Result r;
	r.stamp = this->now();
	r.type = CtlCommandAction::Result::TERMINATE_SUCCESS;
	setSucceeded(r);
	controller_->flash();
}
...
void Ctl::timeoutNavigation()
{
	...
	auto wrench = controller_->wrenchCommandStop(this->now());
...
//------------------------------------------------------------------------------
// 制御目標妥当性確認
bool Ctl::validCommand(const std::shared_ptr<const CtlCommandAction::Goal>& goal) const
{
	...
	std::shared_ptr<ib2_msgs::srv::UpdateParameter::Response> response)
{
	...
	{
		RCLCPP_INFO(this->get_logger(), "%s: Failed", UPDATE_SERVICE.c_str());
...
void Ctl::navinfoCallback(const ib2_msgs::msg::Navigation::SharedPtr msg)
{
	const auto& nav_stamp = *msg;
	...
	try
	{
		...
		else
		{
			...
			if (up_wrench_mode_ == UP_WRENCH_MODE::USE_TORQUE_GAIN_FORWARD_IB2)
			{
				// Use minus torque gain.
				wrench = controller_->wrenchCommand(nav_stamp, p, *body_, -up_torque_gain_);
			}
			else
			{
				// Not Use torque gain.
				wrench = controller_->wrenchCommand(nav_stamp, p, *body_);
			}
...
//------------------------------------------------------------------------------
// Callback of subscribe on the TOPIC_UP_WRENCH_MODE
void Ctl::upWrenchModeCallback(const std_msgs::msg::Int8::SharedPtr msg)
{
	try
	{
		up_wrench_mode_ = static_cast<UP_WRENCH_MODE>(msg->data);
		RCLCPP_INFO(this->get_logger(), "Wrench mode: %d at Ctl::upWrenchModeCallback", static_cast<int>(up_wrench_mode_));
	}
	catch (const std::exception& e)
	{
		std::string what(ib2_mss::Log::caughtException(e.what()));
	...
	{
		RCLCPP_ERROR(this->get_logger(), "caught exception at Ctl::upWrenchModeCallback");
	}
}

//------------------------------------------------------------------------------
// Callback of subscribe on the TOPIC_UP_TORQUE_GAIN
void Ctl::upTorqueGainCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
{
	try
	{
		std::vector<double> std_vector_double = msg->data;
		double* ptr = &std_vector_double[0];
		Eigen::Map<Eigen::Vector3d> vec(ptr, std_vector_double.size());
		up_torque_gain_ = vec;
		RCLCPP_INFO(this->get_logger(), "Get Torque gain: [%f,%f,%f]  at Ctl::upTorqueGainCallback",
			up_torque_gain_(0), up_torque_gain_(1), up_torque_gain_(2));
	}
	catch (const std::exception& e)
	{
		std::string what(ib2_mss::Log::caughtException(e.what()));
	...
	{
		RCLCPP_ERROR(this->get_logger(), "caught exception at Ctl::upTorqueGainCallback");
	}
}
