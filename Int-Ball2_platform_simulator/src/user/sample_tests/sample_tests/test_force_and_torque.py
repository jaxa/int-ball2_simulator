...
class TestForceAndTorqueNode(Node):
    ...
    def __publish_status(self):
        msg = list(self.__status.encode(encoding='utf-8')[:TestForceAndTorqueNode.MAX_MSG_SIZE])
        if len(msg) < TestForceAndTorqueNode.MAX_MSG_SIZE:
            msg.extend([0] * (TestForceAndTorqueNode.MAX_MSG_SIZE - len(msg)))
