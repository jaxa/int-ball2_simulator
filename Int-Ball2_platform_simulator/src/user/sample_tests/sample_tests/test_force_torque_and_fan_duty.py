...
class TestForceTorqueAndFanDutyNode(Node):
    ...
    def __init__(self):
        ...
        self.__force_and_torque_publisher = self.create_publisher(WrenchStamped, '/ctl/wrench', 1)
    ...
    def __publish_status(self):
        msg = list(self.__status.encode(encoding='utf-8')[:TestForceTorqueAndFanDutyNode.MAX_MSG_SIZE])
        if len(msg) < TestForceTorqueAndFanDutyNode.MAX_MSG_SIZE:
            msg.extend([0] * (TestForceTorqueAndFanDutyNode.MAX_MSG_SIZE - len(msg)))
    ...
    def __process_controll_force_and_torque(self):
        ...
        wrench = WrenchStamped(
            ...
            wrench=Wrench(
                force=Vector3(x=0.5, y=0.0, z=0.0),
                torque=Vector3(x=0.5, y=0.0, z=0.0)
            )
        )
        ...
        wrench = WrenchStamped(
            header=Header(stamp=self.get_clock().now().to_msg()),
            wrench=Wrench(
                force=Vector3(x=0.0, y=0.5, z=0.0),
                torque=Vector3(x=0.0, y=0.5, z=0.0)
            )
        )
        self.__force_and_torque_publisher.publish(wrench)
        time.sleep(sleep_duration)
...
def main(args=None):
    ...
    node = TestForceTorqueAndFanDutyNode()
    try:
        rclpy.spin(node)
