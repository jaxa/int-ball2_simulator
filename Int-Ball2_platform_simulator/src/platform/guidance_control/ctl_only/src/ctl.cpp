...
void Ctl::cancelTarget(bool docking)
{
	...
	if (docking)
	{
		...
		auto ipos(ib2::PosAttProfiler::DOCKING_POS::AIP);
		auto iatt(ib2::PosAttProfiler::DOCKING_ATT::RDA);
		auto profmsg2(profiler_->dockingProfile
					 (last_nav_stamp_, ipos, iatt, *body_));
		profile_pub_->publish(profmsg2);
		controller_->flash();

		rclcpp::Time tnav2(last_nav_stamp_.pose.header.stamp);
		auto tbGoal2 = tnav2;
		bool stayGoal2(false);
		auto te2(profiler_->te() + waitCancel_);
		while (tnav2 < te2)
		{
			tnav2 = rclcpp::Time(last_nav_stamp_.pose.header.stamp, RCL_ROS_TIME);
			if (this->now() - tnav2 >= waitCancel_)
			{
				timeoutNavigation();
				break;
			}
			auto fb = profiler_->statesToGoal(last_nav_stamp_);
			if (reachGoal(stayGoal2, tbGoal2, tnav2, fb,
						  tolerance_pos_stop_, tolerance_att_stop_))
				break;
		}
	}
