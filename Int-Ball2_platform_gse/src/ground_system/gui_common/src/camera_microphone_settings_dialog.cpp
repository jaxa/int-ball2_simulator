...
#include "ui_camera_microphone_settings_dialog.h"
...
const QString CameraMicrophoneSettingsDialog::RESOLUTION_KEYWORD_4K = "Four_K";
...
CameraMicrophoneSettingsDialog::CameraMicrophoneSettingsDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::CameraMicrophoneSettingsDialog),
    intballTelemetry_(nullptr)
{
    ...
    // テレメトリの設定をしない場合は非表示.
    ui->groupCameraMicrophoneTelemetry->hide();
...
CameraMicrophoneSettingsDialog::~CameraMicrophoneSettingsDialog()
{
    delete ui;
}

bool CameraMicrophoneSettingsDialog::isValidInput()
{
...
void CameraMicrophoneSettingsDialog::valueChanged(QObject* sender, double arg, QToolButton* clearButton, QLabel* label)
{
...
void CameraMicrophoneSettingsDialog::initialize(IntBallTelemetry* intballTelemetry, TelecommandClient* telecommandClient)
{
    INFO_START_FUNCTION();
    ...
    connect(intballTelemetry_, &IntBallTelemetry::dataChanged,
            this, &CameraMicrophoneSettingsDialog::IntBall2Telemetry_dataChanged);
    ui->groupCameraMicrophoneTelemetry->show();
}

void CameraMicrophoneSettingsDialog::resetValues()
{
...
void CameraMicrophoneSettingsDialog::IntBall2Telemetry_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
    Q_UNUSED(topLeft);
    Q_UNUSED(bottomRight);
    Q_UNUSED(roles);

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_ZOOM))
    {
        ui->labelZoomValue->setText(QString::number(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_ZOOM).toDouble(), 'f', 2));
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_EV))
    {
        ui->labelEVValue->setText(QString::number(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_EV).toDouble(), 'f', 2));
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_WHITE_BALANCE_MODE))
    {
        ui->labelWhiteBalanceValue->setText(getWhiteBalanceModeAsString(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_WHITE_BALANCE_MODE)));
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_RESOLUTION_TYPE))
    {
        ui->labelResolutionValue->setText(
                    cameraConfig_->getQualitySettingAsStringById(
                        intballTelemetry_->data<ib2_msgs::msg::MainCameraResolutionType>(telemetry::Index::CAMERA_MIC_RESOLUTION_TYPE).type));
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_FRAME_RATE))
    {
        ui->labelFrameRateValue->setNum(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_FRAME_RATE).toInt());
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_CAMERA_GAIN))
    {
        ui->labelCameraGainValue->setText(QString::number(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_CAMERA_GAIN).toInt()));
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_SENDING_BIT_RATE))
    {
        ui->labelBitRateValue->setNum(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_SENDING_BIT_RATE).toInt());
    }

    if(intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_MICROPHONE_GAIN))
    {
        ui->labelMicGainValue->setText(QString::number(intballTelemetry_->data(telemetry::Index::CAMERA_MIC_MICROPHONE_GAIN).toDouble(), 'f', 2));
    }
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearZoom_clicked()
{
    ui->doubleSpinBoxZoom->setValue(ui->doubleSpinBoxZoom->minimum());
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearResolution_clicked()
{
    ui->comboBoxResolution->setCurrentIndex(0);
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearEV_clicked()
{
    ui->doubleSpinBoxEV->setValue(ui->doubleSpinBoxEV->minimum());
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearWhiteBalance_clicked()
{
    ui->comboBoxWhiteBalance->setCurrentIndex(0);
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearCameraGain_clicked()
{
    ui->doubleSpinBoxCameraGain->setValue(ui->doubleSpinBoxCameraGain->minimum());
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearFrameRate_clicked()
{
    ui->comboBoxFrameRate->setCurrentIndex(0);
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearBitRate_clicked()
{
    ui->comboBoxBitRate->setCurrentIndex(0);
}

void CameraMicrophoneSettingsDialog::on_toolButtonClearMicrophoneGain_clicked()
{
    ui->doubleSpinBoxMicrophoneGain->setValue(ui->doubleSpinBoxMicrophoneGain->minimum());
}

void CameraMicrophoneSettingsDialog::on_doubleSpinBoxZoom_valueChanged(double arg1)
{
    valueChanged(sender(), arg1, ui->toolButtonClearZoom, ui->labelZoom);
}


void CameraMicrophoneSettingsDialog::on_doubleSpinBoxEV_valueChanged(double arg1)
{
    valueChanged(sender(), arg1, ui->toolButtonClearEV, ui->labelEV);
}


void CameraMicrophoneSettingsDialog::on_comboBoxResolution_currentIndexChanged(int index)
{
    checkFrameRateAndResolution();
    valueChanged(sender(), index, ui->toolButtonClearResolution, ui->labelResolution);
}

void CameraMicrophoneSettingsDialog::on_comboBoxWhiteBalance_currentIndexChanged(int index)
{
    valueChanged(sender(), index, ui->toolButtonClearWhiteBalance, ui->labelWhiteBalance);
}

void CameraMicrophoneSettingsDialog::on_doubleSpinBoxCameraGain_valueChanged(double arg1)
{
    valueChanged(sender(), arg1, ui->toolButtonClearCameraGain, ui->labelCameraGain);
}

void CameraMicrophoneSettingsDialog::checkFrameRateAndResolution()
{
    /*
     * FPS指定値が15を超えている場合はチェック
     */
    if(ui->comboBoxFrameRate->currentData().toInt() > 15)
    {
        if((ui->comboBoxResolution->currentIndex() > 0) &&
                (ui->comboBoxResolution->currentText().contains(RESOLUTION_KEYWORD_4K)))
        {
            // 解像度の変更指定値がFPS15を超えていた場合は選択を変更する.
            ui->comboBoxFrameRate->setCurrentText("15");
        }
        else
        {
            // 解像度の変更が指定されていない場合,テレメトリ値が無いまたは4K画質の場合はFPS15が最大値
            if((intballTelemetry_ == nullptr) ||
                    (!intballTelemetry_->getInsertStatus(telemetry::Index::CAMERA_MIC_RESOLUTION_TYPE))||
                    ui->labelResolutionValue->text().contains(RESOLUTION_KEYWORD_4K))
            {
                // 解像度の変更が指定されていない場合.
                ui->comboBoxFrameRate->setCurrentText("15");
            }
...
void CameraMicrophoneSettingsDialog::on_comboBoxFrameRate_currentIndexChanged(int index)
{
    checkFrameRateAndResolution();
    valueChanged(sender(), index, ui->toolButtonClearFrameRate, ui->labelFrameRate);
}

void CameraMicrophoneSettingsDialog::on_comboBoxBitRate_currentIndexChanged(int index)
{
    valueChanged(sender(), index, ui->toolButtonClearBitRate, ui->labelBitRate);
}

void CameraMicrophoneSettingsDialog::on_doubleSpinBoxMicrophoneGain_valueChanged(double arg1)
{
...
void CameraMicrophoneSettingsDialog::on_settingsSendButton_clicked()
{
    ...
    if(request.size() > 0)
    {
        // ダイアログを前面表示とするため、自身（ダイアログ）を一旦非表示にする.
        this->hide();
        ...
        if(telecommandClient_->sendSetRosParams(request))
        {
           ...
           if(!telecommandClient_->sendUpdateParameter(telecommand::UPDATE_PARAMETER_TARGET::CAMERA_AND_MICROPHONE))
           {
               // 送信失敗.
